(function () {
  ...
  function ensureRecipe(values, ingredients) {
    const existing = findRecipeByName(values.name);
    if (!existing) return RecipeService.createRecipe(values, ingredients);
    const existingType = RecipeService.recipeTypeOf(existing);
    const desiredType = RecipeService.recipeTypeOf({ recipeType: values.recipeType || values.type });
    const hasNewComponents = Array.isArray(existing.components) && existing.components.length;
    if (existingType !== desiredType || !hasNewComponents) {
      return RecipeService.updateRecipe(existing.id, values, ingredients);
    }
    return existing;
  }
  ...
  function ensureMenuItem(values) {
    const existing = findMenuItemByName(values.name);
    if (!existing) return MenuService.createMenuItem(values);
    if (existing.recipeId !== values.recipeId || Number(existing.sellingPrice) !== Number(values.sellingPrice)) {
      return MenuService.updateMenuItem(existing.id, values);
    }
    return existing;
  }
  ...
  function load() {
    const items = {
      ...
      chickenFillet: ensureItem({ name: "Chicken Fillet", sku: "PROT-003", categoryId: "CAT-PROTEIN", primaryUnitId: "UNIT-CS", intermediateUnitId: "UNIT-PK", baseUnitId: "UNIT-EA", intermediateUnitsPerPrimary: 6, baseUnitsPerIntermediate: 24, defaultLocationId: "LOC-WALKIN-FREEZER", purchaseUnitCost: 198.72, minimumLevel: 96, parLevel: 288, maximumLevel: 576 }),
      ...
      mayo: ensureItem({ name: "Mayo", sku: "SAUCE-002", categoryId: "CAT-SAUCE", primaryUnitId: "UNIT-TUB", intermediateUnitId: "", baseUnitId: "UNIT-OZ", baseUnitsPerPrimary: 128, defaultLocationId: "LOC-DRY-STORAGE", purchaseUnitCost: 20.8, minimumLevel: 64, parLevel: 256, maximumLevel: 512 }),
      ketchup: ensureItem({ name: "Ketchup", sku: "SAUCE-003", categoryId: "CAT-SAUCE", primaryUnitId: "UNIT-TUB", intermediateUnitId: "", baseUnitId: "UNIT-OZ", baseUnitsPerPrimary: 128, defaultLocationId: "LOC-DRY-STORAGE", purchaseUnitCost: 22.4, minimumLevel: 64, parLevel: 256, maximumLevel: 512 }),
      seasoning: ensureItem({ name: "Seasoning", sku: "DRY-002", categoryId: "CAT-DRY", primaryUnitId: "UNIT-BAG", intermediateUnitId: "", baseUnitId: "UNIT-OZ", baseUnitsPerPrimary: 80, defaultLocationId: "LOC-DRY-STORAGE", purchaseUnitCost: 32, minimumLevel: 16, parLevel: 80, maximumLevel: 160 }),
      pickles: ensureItem({ name: "Pickles", sku: "PROD-003", categoryId: "CAT-PRODUCE", primaryUnitId: "UNIT-TUB", intermediateUnitId: "", baseUnitId: "UNIT-EA", baseUnitsPerPrimary: 300, defaultLocationId: "LOC-WALKIN-COOLER", purchaseUnitCost: 18, minimumLevel: 150, parLevel: 600, maximumLevel: 900 }),
      ...
      lids: ensureItem({ name: "Drink Lids 20oz", sku: "PACK-002", categoryId: "CAT-PACKAGING", primaryUnitId: "UNIT-CS", intermediateUnitId: "UNIT-PK", baseUnitId: "UNIT-EA", intermediateUnitsPerPrimary: 20, baseUnitsPerIntermediate: 50, defaultLocationId: "LOC-DRY-STORAGE", purchaseUnitCost: 42, minimumLevel: 500, parLevel: 2000, maximumLevel: 4000 }),
      straws: ensureItem({ name: "Straws", sku: "PACK-003", categoryId: "CAT-PACKAGING", primaryUnitId: "UNIT-CS", intermediateUnitId: "UNIT-PK", baseUnitId: "UNIT-EA", intermediateUnitsPerPrimary: 10, baseUnitsPerIntermediate: 500, defaultLocationId: "LOC-DRY-STORAGE", purchaseUnitCost: 38, minimumLevel: 1000, parLevel: 5000, maximumLevel: 10000 }),
      comboBag: ensureItem({ name: "Combo Bag", sku: "PACK-004", categoryId: "CAT-PACKAGING", primaryUnitId: "UNIT-CS", intermediateUnitId: "UNIT-PK", baseUnitId: "UNIT-EA", intermediateUnitsPerPrimary: 10, baseUnitsPerIntermediate: 100, defaultLocationId: "LOC-DRY-STORAGE", purchaseUnitCost: 68, minimumLevel: 300, parLevel: 1200, maximumLevel: 2500 }),
    ...
    [
      [items.chicken, 4, "UNIT-CS"], [items.chickenFillet, 4, "UNIT-CS"], [items.thighs, 3, "UNIT-CS"], [items.oil, 3, "UNIT-CS"],
      ...
      [items.mayo, 3, "UNIT-TUB"], [items.ketchup, 2, "UNIT-TUB"], [items.seasoning, 2, "UNIT-BAG"],
      [items.pickles, 2, "UNIT-TUB"], [items.cups, 3, "UNIT-CS"], [items.lids, 3, "UNIT-CS"],
      [items.straws, 1, "UNIT-CS"], [items.comboBag, 2, "UNIT-CS"], [items.cola, 4, "UNIT-BAG"], [items.sanitizer, 1, "UNIT-CS"]
    ...
    const houseSauce = ensureRecipe({ name: "House Sauce Prep", recipeType: "PREP_ITEM", yieldQuantity: 42, yieldUnitId: "UNIT-OZ" }, [
      { sourceType: "INVENTORY_ITEM", sourceId: items.mayo.id, quantity: 32, unitId: "UNIT-OZ" },
    ...
    const chickenCombo = ensureRecipe({ name: "Chicken Sandwich Combo", recipeType: "COMBO" }, [
      ...
      { sourceType: "INVENTORY_ITEM", sourceId: items.comboBag.id, quantity: 1, unitId: "UNIT-EA" }
    ]);
    ...
    ensureSale({ menuItemId: menuItems[3].id, quantitySold: 45, date: today() });
    ...
    return { items: Object.keys(items).length, recipes: 5, menuItems: menuItems.length };
  }
