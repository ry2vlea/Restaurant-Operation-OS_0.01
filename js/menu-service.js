(function () {
  ...
  const save = (items) => { localStorage.setItem(key, JSON.stringify(items)); window.dispatchEvent(new CustomEvent("menu:changed")); return items; };
  ...
  function getMenuItemById(id) { return read().find((item) => item.id === id) || null; }
  ...
  function calculateAvailability(menuItem) {
    ...
    const ingredients = RecipeService.getRecipeIngredients(recipe.id);
    const availability = ingredients.map((ingredient) => {
      const stock = InventoryService.getItemStock(ingredient.inventoryItemId);
      ...
      return { ingredient, item: InventoryService.getItemById(ingredient.inventoryItemId), stock, perServingBaseQuantity, servings };
    ...
    const status = menuItem.status === "MANUALLY_UNAVAILABLE"
      ...
      : "AVAILABLE";
    return { servings, status, limitingIngredient: limiting?.item || null, ingredientAvailability: availability };
  }
  ...
  window.MenuService = { getMenuItems, getMenuItemById, createMenuItem, updateMenuItem, setManualAvailability, calculateAvailability, calculateMenuMetrics };
})();
