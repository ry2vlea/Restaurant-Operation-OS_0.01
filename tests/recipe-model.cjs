...
// Menu lifecycle keeps recorded sales and IDs intact.
load('js/business-performance-service.js');
const historicalSales = JSON.stringify(S.getSales('2026-09-01'));
const historicalMetrics = JSON.stringify(S.calculateMetrics('2026-09-01', '2026-09-01'));
const historicalUsage = JSON.stringify(context.TheoreticalUsageService.calculateForDate('2026-09-01'));
const movementsBeforeMenuEdit = data.get('inventoryMovements');
M.updateMenuItem(menu.id, { name: 'Renamed combo', sellingPrice: 22, limitedThreshold: 0 });
assert.equal(M.getMenuItemById(menu.id).limitedThreshold, 0);
assert.throws(() => M.updateMenuItem(menu.id, { sellingPrice: -1 }), /price/i);
assert.throws(() => M.updateMenuItem(menu.id, { name: ' ' }), /name/i);
M.deleteMenuItem(menu.id);
assert(!M.getMenuItems().some(item => item.id === menu.id));
assert(M.getMenuItemById(menu.id).deletedAt);
assert.throws(() => S.saveSale({ date: '2026-09-02', menuItemId: menu.id, quantitySold: 1 }), /not found/i);
assert.throws(() => M.updateMenuItem(menu.id, { name: 'Restore' }), /not found/i);
const replacement = M.createMenuItem({ name: 'New combo', recipeId: combo.id, sellingPrice: 12 });
assert.notEqual(replacement.id, menu.id);
assert.equal(JSON.stringify(S.getSales('2026-09-01')), historicalSales);
assert.equal(JSON.stringify(S.calculateMetrics('2026-09-01', '2026-09-01')), historicalMetrics);
assert.equal(JSON.stringify(context.TheoreticalUsageService.calculateForDate('2026-09-01')), historicalUsage);
assert.equal(data.get('inventoryMovements'), movementsBeforeMenuEdit);
assert.equal(S.getRecordingStatus('2026-09-02').salesRecorded, false);
S.saveDaySummary({ date: '2026-09-02', transactions: 0 });
assert.equal(S.getRecordingStatus('2026-09-02').transactionsRecorded, true);
const B = context.BusinessPerformanceService;
assert.equal(B.getLaborSummary('2026-09-01').recorded, false);
B.saveRecord({ date: '2026-09-01', laborDollars: 0, laborHours: 0 });
assert.equal(B.getLaborSummary('2026-09-01').recorded, true);
assert.equal(B.getLaborSummary('2026-09-01').laborDollars, 0);
console.log('PASS: menu edits/deletion, unique IDs, sales snapshots, inventory isolation, and dashboard recording status');
